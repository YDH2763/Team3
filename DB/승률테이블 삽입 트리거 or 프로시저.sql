...
BEGIN
    ...
    SET _S_POSITION = CASE (NEW.S_POSITION)(SELECT NEW.S_POSITION);
    SET _S_WIN = IFNULL((SELECT NEW.S_WIN),0);
    SET _S_COUNT = IFNULL((SELECT NEW.S_COUNT),0);
...
BEGIN
    ...
    UPDATE WINNING_RATE
	SET
		WR_BLACK = ((_S_WIN)/(_S_COUNT))*100 WHERE _S_POSITION = "BLACK" AND WR_U_NAME = _S_U_NAME;
...
BEGIN
    ...
    UPDATE WINNING_RATE
	SET
		WR_WHITE = ((_S_WIN)/(_S_COUNT))*100 WHERE _S_POSITION = "WHITE" AND WR_U_NAME = _S_U_NAME;
